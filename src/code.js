...
const bgBrandColor = getComputedStyle(
  document.documentElement
).getPropertyValue("--figma-color-bg-brand");
const backgroundDisabledColor = getComputedStyle(
  document.documentElement
).getPropertyValue("--figma-color-bg-disabled");
...
const feature = {
  ...
  drawChartPreview() {
    ...
    for (let i = 0; i < previewCoordinates.length - 1; i++) {
      ctx.lineTo(
        gridSize * previewCoordinates[i].x,
        gridSize * previewCoordinates[i].y
      );

      // Moving context 1 step further if the function is discontinuous and we expect an abrupt change
      if (
        (previewCoordinates[i].y < -3 || previewCoordinates[i].y > 3) &&
        (previewCoordinates[i + 1].y < -3 || previewCoordinates[i + 1].y > 3)
      ) {
        ctx.moveTo(
          gridSize * previewCoordinates[i + 1].x,
          gridSize * previewCoordinates[i + 1].y
        );
      }
    }
    ctx.stroke();
